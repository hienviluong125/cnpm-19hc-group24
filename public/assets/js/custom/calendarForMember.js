  ...
  CalendarApp.prototype.init = function () {
    ...
    $this.$calendarObj = $this.$calendar.fullCalendar({
      ...
      dayClick: function(date) {
        $('#trainerList').modal('show');
        $('#trainerListLabel').html(`Book a trainer on ${date.format()}`);
        $.ajax({
          method: 'GET',
          url: `/trainers/${date.format()}`,
          contentType: 'json',
          success: function(result) {
            const { trainers } = result;
            $('#trainerListRow').html('');

            if(trainers.length > 0) {
              for(let trainer of trainers) {
                $('#trainerListRow').append(`
                <div class="col-12">
                  <div class="d-flex justify-content-between mt-1 mb-2">
                  <h5 class="mt-1">${trainer.first_name} ${trainer.last_name}</h5>
                  <a href="/trainers/${trainer.id}/book?date=${date.format()}" class="btn btn-primary btn-sm">Book now</a>
                </div>
                `)
              }
            }
            else {
              $('#trainerListRow').append('<h4 class="col-md-12">Everyone seems to be busy on this day</h3>')
            }
          },
          error: function(err) {
            console.log( {err});
